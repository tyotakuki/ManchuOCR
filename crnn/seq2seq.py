...
class CNN(nn.Module):
    ...
    def forward(self, input):
        # [n, channel_size, 64, 480] -> [n, 512, 1, 61]
        conv = self.cnn(input)
        a,b,c,d = conv.size()
        conv = conv.view(a,b*c,1,d)
        return conv
...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=61):
...
class Encoder(nn.Module):
    def __init__(self, channel_size, hidden_size):
        ...
        self.rnn = nn.Sequential(
            BidirectionalLSTM(1536, hidden_size, hidden_size),
